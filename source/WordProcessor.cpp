...
std::queue<std::string> WordProcessor::extractSeedUrlsFromString(const std::string &string)
{
    std::queue<std::string> results_queue{};
    const std::regex pattern{R"(\"(.*?)\")"};
    std::sregex_iterator end;

    auto process_match = [&](std::sregex_iterator &it)
    {
        while (it != end)
        {
            results_queue.push(WordProcessor::normalizeQuotedPhrase(it->str()));
            ++it;
        }
    };
    try
